...
@app.post("/candidates")
def create_candidate(c_req: CandidateReq):
    return Candidate.create(c_req)


@app.get("/candidates/{c_id}")
def get_candidate(c_id: str):
    return Candidate.get(c_id)


@app.get("/candidates")
def list_candidates(recruitment_status: Optional[RecruitmentStatusEnum] = None):
    return Candidate.list(recruitment_status=recruitment_status)


@app.put("/candidates/{c_id}")
def update_candidate(c_id: str, c_req: CandidateReq):
